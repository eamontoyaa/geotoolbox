...
class Cell:
    ...
    def __init__(
        ...
        # self.exp = int(exp)
        self.exp = float(exp)  # retain decimals (NumPy handles float powers)
        ...
        if (
            type(startcell) == bool
        ):  # check, if start cell exist (start cell is release point)
    ...
    def calc_fp_travelangle(self):
        ...
        for parent in self.parent:
            ...
            dist_min.append(
                math.sqrt(dx**2 + dy**2) * self.cellsize + parent.min_distance
            )
    ...
    def calc_z_delta(self):
        ...
        ds = np.array(
            [[np.sqrt(2), 1, np.sqrt(2)], [1, 0, 1], [np.sqrt(2), 1, np.sqrt(2)]]
        )
        ...
        self.z_delta_neighbour[self.z_delta_neighbour > self.max_z_delta] = (
            self.max_z_delta
        )
        ...
        if self.is_start:
            ...
            for parent in self.parent:
                ...
                self.no_flow[dy + 1, dx + 1] = (
                    0  # 3x3 Matrix of ones, every parent gets a 0, so no flow to a parent field.
                )
    ...
    def calc_distribution(self):
        ...
        if np.sum(self.r_t) > 0:
            self.dist = (
                (self.persistence * self.r_t)
                / np.sum(self.persistence * self.r_t)
                * self.flux
            )
