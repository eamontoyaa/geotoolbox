...
def calculation(optTuple):
    ...
    sl_travelangle_array = (
        np.zeros_like(dem, dtype=np.float32) * 90
    )  # sl = Straight Line
    ...
    np.save(
        temp_dir + "./res_z_delta_{}_{}".format(optTuple[0], optTuple[1]), z_delta_array
    )
    np.save(
        temp_dir + "./res_z_delta_sum_{}_{}".format(optTuple[0], optTuple[1]),
        z_delta_sum,
    )
    np.save(temp_dir + "./res_flux_{}_{}".format(optTuple[0], optTuple[1]), flux_array)
    np.save(
        temp_dir + "./res_count_{}_{}".format(optTuple[0], optTuple[1]), count_array
    )
    np.save(
        temp_dir + "./res_fp_{}_{}".format(optTuple[0], optTuple[1]),
        fp_travelangle_array,
    )
    np.save(
        temp_dir + "./res_sl_{}_{}".format(optTuple[0], optTuple[1]),
        sl_travelangle_array,
    )
    np.save(
        temp_dir + "./res_backcalc_{}_{}".format(optTuple[0], optTuple[1]), backcalc
    )
